...
https://github.com/fengliu90/DK-for-TST/blob/master/utils_HD.py
...
def Pdist2(x, y):
    ...
    # print("x.shape:{}, x_norm: {}, {}".format(x.size(), x_norm.size(), (x ** 2).sum(1).size()))
    if y is not None:
...
def Pdist1(x, y):
    """compute the paired distance between x and y."""
    if type(x) == type(torch.randn(1,1)):
        x = x.to(torch.device('cpu')).numpy()
    if y is not None:
        if type(y) == type(torch.randn(1,1)):
            y = y.to(torch.device('cpu')).numpy()
    else:
        y = x
    Pdist = pairwise_distances(x, y, metric='l1')
    Pdist = torch.from_numpy(Pdist)
    Pdist[Pdist<0]=0
    return Pdist
...
def h1_mean_var_gram(Kx, Ky, Kxy, is_var_computed, use_1sample_U=True):
    ...
    # print("Kx : {}, Ky: {}, Kxy: {}".format(Kx.size(), Ky.size(), Kxy.size()))
...
def MMDu_IMQ(Fea, len_s, Fea_org, sigma, sigma0=0.1, epsilon = 10**(-10), is_smooth=True, is_var_computed=True, use_1sample_U=True):
    """compute value of deep-kernel MMD and std of deep-kernel MMD using merged data."""
    X = Fea[0:len_s, :] # fetch the sample 1 (features of deep networks)
    Y = Fea[len_s:, :] # fetch the sample 2 (features of deep networks)
    X_org = Fea_org[0:len_s, :] # fetch the original sample 1
...
def MMDu_laplace(Fea, len_s, Fea_org, sigma, sigma0=0.1, epsilon = 10**(-10), is_smooth=True, is_var_computed=True, use_1sample_U=True):
    """compute value of deep-kernel MMD and std of deep-kernel MMD using merged data."""
    X = Fea[0:len_s, :] # fetch the sample 1 (features of deep networks)
    Y = Fea[len_s:, :] # fetch the sample 2 (features of deep networks)
    X_org = Fea_org[0:len_s, :] # fetch the original sample 1
...
def TST_MMD_kernel_option(Fea, N_per, N1, Fea_org, sigma, sigma0, alpha, device, dtype, kernel_option='laplace'):
    """run two-sample test (TST) using ordinary Gaussian kernel."""
    mmd_vector = np.zeros(N_per)
    if kernel_option == 'laplace':
        TEMP = MMDu_laplace(Fea, N1, Fea_org, sigma, sigma0, is_smooth=False)
    elif kernel_option == 'imq':
        TEMP = MMDu_IMQ(Fea, N1, Fea_org, sigma, sigma0, is_smooth=False)
    else:
        raise Exception(f"kernel option:{kernel_option} is not implemented")
